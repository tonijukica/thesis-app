...
async function verifyPackedAttestation(
  ctapCredentialResponse: any,
  clientDataJSON: string
) {
  const authenticatorDataStruct = parseAuthData(
  ...
  if (ctapCredentialResponse.attStmt.x5c) {
    const leafCert = base64ToPem(
      ctapCredentialResponse.attStmt.x5c[0].toString('base64')
    );
    ...
    const publicKeyCose = cbor.decodeAllSync(
      authenticatorDataStruct.COSEPublicKey
    )[0];
