...
function prepareInputData(data: any) {
  return data.map((rawProject: any) => {
    ...
    const rawStudentUsernames = rawProject[
      process.env.STUDENTS_USERNAMES!
    ].split(', ');
...
function prepareLegacyInputData(data: any) {
  ...
  data.forEach((el: any) => {
    const project = data.filter(
      (pro: any) => pro['Project ID'] === el['Project ID']
    );
    ...
    const exists = projects.find(
      (el: any) => el.github_url === validProject.github_url
    );
    if (!exists) projects.push(validProject);
  });
  return projects;
}
...
async function getFileContent(fileName: Blob) {
  ...
  const legacy = legacyMode === 'true';
  let fileData;
  if (legacy) fileData = prepareLegacyInputData(parseInput(rawFileData));
  else fileData = prepareInputData(parseInput(rawFileData));
  return fileData;
}
