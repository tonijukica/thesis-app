...
function userCommitsGraph(commits: any) {
  ...
  commits.forEach((commit: any) => {
    ...
    if (graphCommits[userIndex] == null) {
      ...
      graphCommits[userIndex].commits.push(commit);
  });
  let labels: any = {};
  graphCommits.map((el: any) => {
    labels[el.label] = 0;
  });
  const history: any = [];
  commits.forEach((commit: any) => {
...
export { getUserRepoName, dataExtract, formatDate, projectStanding, userCommits, userCommitsGraph };
