...
const ProjectList: FunctionComponent<ProjectListProps> = ({ courseId }) => {
  ...
  const sortStyle = (sort: boolean | null) => {
    if (sort === true)
      return classnames(classes.expandIcon, classes.expandIconDown);
    else if (sort === false)
      return classnames(classes.expandIcon, classes.expandIconUp);
    else return '';
  };
  const sortIndicator = (sort: boolean | null) => {
    if (sort !== true && sort !== false)
      return <UnfoldMoreIcon style={{ width: '20px' }} />;
    else return <ExpandMoreIcon fontSize="small" className={sortStyle(sort)} />;
  };
  ...
  return (
    <>
      <Context.Provider value={{ state, dispatch }}>
        ...
        >
          <Grid container item xs={4}>
            ...
            <FormControlLabel
              ...
              control={
                <Switch
                  checked={state.standing}
                  color="secondary"
                  onClick={handleStandingMode}
                />
              }
              label="Standing"
              labelPlacement="end"
        ...
        <ProjectDialog courseId={courseId} />
        <BulkProjectDialog courseId={courseId} />
